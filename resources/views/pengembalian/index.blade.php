    ...
    <div class="mx-auto max-w-7xl">
        ...
        @if (session('success'))
            <div class="mb-6 flex items-center gap-4 rounded-2xl border border-emerald-200 bg-emerald-50/80 px-5 py-4 shadow-sm backdrop-blur-md">
                ...
                <div>
                    <p class="text-xs font-bold uppercase tracking-wider text-emerald-900">Proses Berhasil</p>
        ...
        <div class="mb-8 grid gap-5 sm:grid-cols-3">
            ...
            {{-- CARD 1: MENUNGGU --}}
            <div class="group relative overflow-hidden rounded-3xl border border-slate-100 bg-gradient-to-br from-white via-slate-50/50 to-orange-50/30 p-6 shadow-sm transition-all duration-300 hover:-translate-y-1 hover:border-orange-200 hover:shadow-xl hover:shadow-orange-500/10">
                <div class="absolute -right-8 -top-8 h-28 w-28 rounded-full bg-orange-400/10 blur-2xl transition-all duration-500 group-hover:scale-150 group-hover:bg-orange-400/20"></div>
                <div class="relative z-10">
                    <div class="mb-6 flex items-center justify-between">
                        ...
                        <span class="rounded-full bg-amber-50 px-3 py-1 text-[11px] font-bold tracking-wide text-amber-700 ring-1 ring-inset ring-amber-600/20">MENUNGGU</span>
                    </div>
                    <p class="text-xs font-semibold tracking-wide uppercase text-slate-400">Pengajuan Menunggu</p>
                    <h2 class="mt-1 text-4xl font-black tracking-tight text-amber-600">
                        {{ $peminjamans->where('status', 'menunggu')->count() }}
                    </h2>
                    <p class="mt-3 text-xs text-slate-500">Menunggu persetujuan admin</p>
                </div>
            </div>
            ...
            <div class="group relative overflow-hidden rounded-3xl border border-slate-100 bg-gradient-to-br from-white via-slate-50/50 to-blue-50/30 p-6 shadow-sm transition-all duration-300 hover:-translate-y-1 hover:border-blue-200 hover:shadow-xl hover:shadow-blue-500/10">
                ...
                <div class="relative z-10">
                    ...
                    <p class="text-xs font-semibold tracking-wide uppercase text-slate-400">Buku Sedang Dipinjam</p>
                    <h2 class="mt-1 text-4xl font-black tracking-tight text-blue-600">
                        {{ $peminjamans->where('status', 'dipinjam')->count() }}
                    </h2>
                    <p class="mt-3 text-xs text-slate-500">Belum dikembalikan oleh user</p>
                </div>
            </div>
            ...
            <div class="group relative overflow-hidden rounded-3xl bg-slate-900 p-6 text-white shadow-lg transition-all duration-300 hover:-translate-y-1 hover:shadow-2xl">
                ...
                <div class="relative z-10">
                    <div class="mb-6 flex items-center justify-between">
                        ...
                        <span class="rounded-full border border-emerald-400/30 bg-emerald-500/10 px-3 py-1 text-[11px] font-bold tracking-wide text-emerald-300">ONLINE</span>
                    </div>
                    <p class="text-xs font-semibold tracking-wide uppercase text-slate-400">Sistem Perpustakaan</p>
                    <h2 class="mt-1 text-2xl font-black tracking-tight text-white">MI Al Falahiyyah</h2>
                    <p class="mt-3 text-xs text-slate-400">Pengelolaan perpustakaan digital</p>
                </div>
            </div>
        ...
        <div class="overflow-hidden rounded-3xl border border-slate-100 bg-white shadow-sm">
            ...
            <div class="flex flex-col justify-between gap-4 border-b border-slate-100 p-6 md:flex-row md:items-center">
                ...
                <div class="flex items-center gap-3 self-start rounded-2xl bg-slate-50 px-4 py-2 ring-1 ring-slate-200/60 md:self-auto">
                    ...
                    <span class="font-extrabold text-emerald-700 text-sm">{{ $peminjamans->total() }}</span>
                </div>
            </div>
                            ...
                            <tr class="transition duration-150 hover:bg-emerald-50/30">
                                ...
                                <td class="px-6 py-4">
                                    <div class="flex items-center gap-3">
                                        ...
                                        <div class="min-w-0">
                                            <p class="truncate font-bold text-slate-800 text-sm">{{ $peminjaman->user->name }}</p>
                                            <p class="truncate text-[11px] text-slate-400">{{ $peminjaman->user->email }}</p>
                                        </div>
                                    </div>
                                </td>
                                    ...
                                    <div class="flex items-center gap-3">
                                        <div class="h-12 w-9 shrink-0 overflow-hidden rounded-xl border border-slate-200 bg-slate-100 shadow-sm">
                                            @if ($peminjaman->buku->sampul)
                                                <img src="{{ asset('storage/' . $peminjaman->buku->sampul) }}" class="h-full w-full object-cover" alt="{{ $peminjaman->buku->judul_buku }}">
                                            @else
                                        ...
                                        <div class="min-w-0">
                                            <p class="max-w-[200px] truncate font-bold text-slate-800 text-sm">{{ $peminjaman->buku->judul_buku }}</p>
                                            <p class="text-[11px] text-slate-400">Kode: <span class="font-mono text-slate-600">{{ $peminjaman->buku->kode_buku }}</span></p>
                                        </div>
                                    </div>
                                </td>
                                ...
                                <td class="px-6 py-4">
                                    <div class="inline-flex items-center gap-2 rounded-xl bg-amber-50 px-3 py-1.5 text-amber-700 ring-1 ring-inset ring-amber-600/20">
                                        ...
                                        <span class="font-bold">{{ \Carbon\Carbon::parse($peminjaman->tanggal_jatuh_tempo)->format('d M Y') }}</span>
                                    </div>
                                </td>
                                ...
                                <td class="px-6 py-4 text-right">
                                    ...
                                    @elseif ($peminjaman->status === 'dipinjam')
                                        <form action="{{ route('pengembalian.kembalikan', $peminjaman) }}" method="POST" onsubmit="return confirm('Yakin buku ini sudah dikembalikan?')">
                                            @csrf
                                            <button type="submit" class="inline-flex items-center gap-1.5 rounded-xl bg-emerald-600 px-4 py-2 text-xs font-bold text-white shadow-md shadow-emerald-600/20 transition hover:bg-emerald-700 hover:scale-105">
                        ...
                        @empty
                            <tr>
                                <td colspan="6" class="px-6 py-16 text-center">
                                    ...
                                    <h3 class="mt-4 text-base font-bold text-slate-800">Tidak Ada Data Peminjaman</h3>
                                    <p class="mt-1 text-xs text-slate-400">Belum ada pengajuan atau peminjaman aktif saat ini.</p>
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
        ...
        <div class="mt-6 flex items-center gap-3 rounded-2xl border border-slate-100 bg-white p-4 shadow-sm">
            <div class="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-amber-50 text-amber-600 text-sm">
                💡
            </div>
